...
def list_modules(course_id: int) -> List[ModuleRead]:
    ...
    return data.db.list_modules(course_id)
...
def delete_module(course_id: int, module_id: int) -> Response:
...
def list_quiz_questions(course_id: int) -> List[QuizQuestionRead]:
    _ensure_course_exists(course_id)
    return data.db.list_quiz_questions(course_id)
...
def delete_quiz_question(course_id: int, question_id: int) -> Response:
...
def attempt_quiz(course_id: int, payload: QuizAttempt) -> QuizResult:
    _ensure_course_exists(course_id)
    questions = data.db.list_quiz_questions(course_id)
    if not questions:
