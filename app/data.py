...
class Database:
    ...
    def list_courses(self) -> List[Course]:
        """Return all courses sorted by their identifier."""
        return [self.courses[c_id] for c_id in sorted(self.courses)]
    ...
    def list_modules(self, course_id: int) -> List[Module]:
        """Return modules for a course ordered by their identifier."""
        return [
            module
            for module in sorted(self.modules.values(), key=lambda module: module.id)
    ...
    def list_quiz_questions(self, course_id: int) -> List[QuizQuestion]:
        """Return quiz questions for a course ordered by identifier."""
        return [
            question
            for question in sorted(self.quiz_questions.values(), key=lambda question: question.id)
